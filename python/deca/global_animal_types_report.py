...
def get_value_for_property(node: RtpcNode, prop_name: str, data_type="utf-8"):
    for property in node.prop_table:
        property_name = fnm.lookup(hash32=property.name_hash)
        if property_name == prop_name:
            return property.data.decode(data_type)
...
class Fur:
    ...
    def _get_name(self, variant_node) -> None:
        for property in reversed(variant_node.prop_table):
            value = property.data
            if isinstance(value, bytes):
                value = property.data.decode("utf-8")
    ...
    def _get_id(self, variant_node: RtpcNode) -> None:
        for property in variant_node.prop_table:
            property_name = fnm.lookup(hash32=property.name_hash)
            if property_name == "_object_id":
                self.id = property.data
    ...
    def _get_gender(self, variant_node: RtpcNode) -> None:
        if type(variant_node.prop_table[3].data) == int:
            ...
            i = 4
        gender = variant_node.prop_table[i].data
        if gender == 0:
            self.gender = "both"
        if gender == 1:
            self.gender = "male"
